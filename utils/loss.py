...
def warp_images_with_flow(images, flow):
    N, C, H, W = images.size()
    assert flow.size()[0] == N, 'Number of images and flows should ' \
                                f'be the same {N} vs {flow.size()[0]}'
    assert flow.size()[1] == 2, 'Flow should contain 2 channels (dx and dy)'
    assert flow.size()[2] == H, 'Height of images and flows should ' \
                                f'be the same {H} vs {flow.size()[2]}'
    assert flow.size()[3] == W, 'Width of images and flows should ' \
                                f'be the same {W} vs {flow.size()[3]}'
    # H, W
...
def contrast_loss(events, flow, ts):
    ...
    for t in [0, ts]:
        for p in [-1, 1]:
            img = contrast_image(events, flow, t, p)
            loss += img.pow_(2)
    return loss
