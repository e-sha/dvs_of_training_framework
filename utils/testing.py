...
def evaluate(of, events, frames, gt,
    ...
    Args:
        ...
        events (list): a list of sorted events in form of [x, y, t, p], where each component is an array
        frames (list): a list of timestamp pairs that generate frame in form of [(start_ts, stop_ts)]
        gt (dict): a dictionary with keys: 'timestamps', 'x_flow_dist', 'y_flow_dist'
        event_preproc_fun (callable): a function to preprocess a set of events. For example, it may restrict events to some bounding box inside the image. Default: None
        pred_postproc_fun (callable): a function to postprocess predicted flow. For example it may crop box from the flow map. Default: None
        gt_proc_fun (callable): a function to preprocess true flow. For example it may crop box from the flow map. Default: None
        is_car (bool): Indicator of sequence captured from car. Default: False
        log (bool): the function prints intermediate statistics if True. Default: False
    '''

    ev_pre_fun = lambda x: x if event_preproc_fun is None else event_preproc_fun(x)
    fl_post_fun = lambda x: x if pred_postproc_fun is None else pred_postproc_fun(x)
    gt_post_fun = lambda x: x if gt_proc_fun is None else gt_proc_fun(x)
    ...
    for i, (e, start, stop) in enumerate(frame_generator(events, frames)):
        ...
        AEE, percent_AEE, n_points = flow_error_dense(gt_flow, flow, event_count_image, is_car)
        AEE_sum += AEE
        ...
        if log and n % 100 == 0:
            print('-------------------------------')
            print(f'Iter: {n}')
            print(f'Mean max flow: {max_flow_sum/n:.2f}, mean min flow: {min_flow_sum/n:.2f}')
            print(f'Mean AEE: {AEE_sum/n:.2f}, mean %AEE: {percent_AEE_sum/n:.2f}, #pts: {n_points},')
