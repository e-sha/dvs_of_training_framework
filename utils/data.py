...
class EventCrop:
    ''' Leaves only events in the specified bounding box and modifies pixel coordinates
    '''
    ...
    def __call__(self, events, box=None):
        ...
        return np.array([x[mask] - box[1], y[mask] - box[0], t[mask], p[mask]]).T
...
class ImageRandomCrop:
    ...
    def __call__(self, img, box=None):
        if box is None:
            start = list(map(lambda x, y: self.__randint(x - y), img.shape[:2], self.shape))
            box = start + list(self.shape)
        res = img[box[0] : box[0] + box[2],
                box[1] : box[1] + box[3]]
        if self.return_box:
...
def RandomRotation(interval, shape):
    ...
    x, y = map(lambda x, s: x.astype(float) - s, (x, y), (shape[1] / 2, shape[0] / 2))
    ...
    assert interval[0] <= interval[1], "The first interval should be lower or equal to the second"
    ...
    def rotation(image1, image2, events, angle=None):
        if angle is None:
            angle = np.random.rand() * (interval[1] - interval[0]) + interval[0]
        ...
        rimage1, rimage2 = map(lambda x: x.reshape(image1.shape), (rimage1, rimage2))

        ## rotate events
        revents = event_map(events.astype(np.float32).copy(), image1.shape,
                cur_ridx.astype(np.uint64), cur_idx.astype(np.uint64))
